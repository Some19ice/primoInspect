...
interface UseEvidenceUploadOptions {
  inspectionId: string
  questionId?: string // NEW: Link evidence to specific checklist question
...
export function useEvidenceUpload({
  ...
  const uploadSingleFile = useCallback(
    async (evidenceFile: EvidenceFile): Promise<boolean> => {
      try {
  ...
  const uploadAll = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      console.log(
        `Upload completed: ${results.filter(Boolean).length}/${results.length} successful`
      )
    } finally {
